...
public class CassieSourceDestCarriers extends Configured implements Tool {
    public static final Log LOG = LogFactory.getLog(CassieSourceDestCarriers.class);
    
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new CassieSourceDestCarriers(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        String query = "UPDATE mp2.source_dest_carrier_ontime_arrivals SET "
        		+ "ontime_arrivals = ?, origin_name = ?, destination_name = ?, airline_name = ?";
        CqlConfigHelper.setOutputCql(jobA.getConfiguration(), query);
        ConfigHelper.setOutputColumnFamily(jobA.getConfiguration(), "mp2", "source_dest_carrier_ontime_arrivals");
        ...
        jobA.setJarByClass(CassieSourceDestCarriers.class);
    ...
    public static class AirportCountReduce extends Reducer<Text, IntWritable, Map<String, ByteBuffer>, List<ByteBuffer>> {
        ...
        public void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
        	...
        	keys.put("airline_id", ByteBufferUtil.bytes(keyValues[2]));
        	...
        	String destinationAirportName = airportIdToNameMap.get(keyValues[1]);
        	String airlineName = airlineIdToNameMap.get(keyValues[2]);
        	variableValues.add(ByteBufferUtil.bytes(Integer.toString(count)));
